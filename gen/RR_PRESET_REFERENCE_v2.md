...
## 1a. Logging scope — settings tables vs master data

For the **report-settings logging** solution only tables that hold **per-preset
configuration** (group **a**) need change tracking. Dictionary / master-data
tables (group **b**) and runtime / temp tables are excluded.

### (a) Settings tables — **need logging**

These tables store data that changes when a user creates, edits, or deletes a
preset. All are scoped by `KOD_PRESET` (directly or through a parent).

| # | Table | Link to `RR_PRESET` | Role | Rows (approx) |
|---|-------|----------------------|------|---------------|
| 1 | `RR_PRESET` | — (root) | Preset header | 963 |
| 2 | `RR_PRESETOPTION` | FK `KOD_PRESET` | Saved option values per preset | 23 254 |
| 3 | `RR_GEN_PRESET_COLS` | FK `KOD_PRESET` | Column visibility / format per preset | 0 |
| 4 | `RR_PRESET_COLUMNS` | FK `KOD_PRESET` | Serialized UI table state (BLOB) per preset | 0 |
| 5 | `RR_PRESET_COMP` | FK `KOD_PRESET_SIMPLE`, `KOD_PRESET_COMP` | Composite-preset ↔ simple-preset links | 11 |
| 6 | `RR_GEN_KODES` | FK `KOD_PRESET` | Generator / grouping codes per preset | 0 |
| 7 | `RR_PRESETWND` | Column `KOD_PRESET` (no declared FK) | Window record per preset | 0 |
| 8 | `RR_PRESETWNDSETUP` | Via `RR_PRESETWND.KOD_WND` | Key / value UI setup per window | 0 |
| 9 | `RR_PRESETENTITY` | Column `KOD_PRESET` (no declared FK) | Preset ↔ entity-element assignments | 0 |

Hierarchy:

```
RR_PRESET  (KOD_PRESET)          ← root
├── RR_PRESETOPTION              ← FK KOD_PRESET
├── RR_GEN_PRESET_COLS           ← FK KOD_PRESET
├── RR_PRESET_COLUMNS            ← FK KOD_PRESET
├── RR_PRESET_COMP               ← FK KOD_PRESET_SIMPLE / KOD_PRESET_COMP
├── RR_GEN_KODES                 ← FK KOD_PRESET
├── RR_PRESETWND                 ← KOD_PRESET (column only)
│   └── RR_PRESETWNDSETUP        ← FK KOD_WND
└── RR_PRESETENTITY              ← KOD_PRESET (column only)
```

### (b) Master-data / dictionary tables — **no logging**

These define the *structure* of available options, column catalogs, and subject
types. They change rarely and are not per-preset.

| Table | Rows | Role |
|-------|------|------|
| `RR_PRESETSUBJECT` | 18 | Report subject / template types |
| `RK_PRESETSUBJOPTION` | 682 | Option definitions per subject |
| `RK_OPTION_TYPE` | 31 | Option-type dictionary |
| `RK_PRESETFLTROPTION` | 58 | Filter SQL / LOV metadata per option |
| `RR_SUBJOPTION` | 2 419 | Subject ↔ allowed-option mapping |
| `RR_GEN_COLS` | 216 | Column catalog per subject |
| `RR_COL_POPT` | 218 | Column ↔ option cross-reference |

### Out of scope

| Table(s) | Reason |
|----------|--------|
| `RR_PRESETOPTION_TMP` | Staging / temporary (no tablespace, GTT-like) |
| `RR_IMPORT` | Import jobs — separate domain (450 K rows) |
| `RR_COMPREP_*`, `RR_GENCOMPREP_*` | Composite report generation output |
| `RR_GEN_CALC_*`, `RR_GEN_CODES_T`, `RR_GEN_DIM_*`, `RR_GEN_HEADERS`, `RR_GEN_OBJ_T` | Report generation runtime / temp tables |
| `UR_LIST_OBLIGATOR` | Unrelated domain |
